...
rem create the webapps loction and copy ci_cd_pipeline into webapps location 
IF NOT EXIST %tomee_path%tomee (
	md %tomee_path%tomee\webapps\3dspace\webapps
	xcopy %source_path%\ci_cd_pipeline.* %tomee_path%tomee\webapps\3dspace\webapps /i /y
)
...
IF EXIST %tomee_path%tomee (
	Robocopy /MIR %source_path%\jar_war_file %tomee_path%tomee\webapps\3dspace\webapps
	
)

FOR /L %%A IN (1,1,%number_of_tomee%) DO (
    IF EXIST %tomee_path%tomee%%A (
		Robocopy /MIR %source_path%\jar_war_file %tomee_path%tomee%%A\webapps\3dspace\webapps
	)
)
